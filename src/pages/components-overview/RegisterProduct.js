...
const RegisterProduct = () => {
  ...
  useEffect(() => {
    if (itemId) {
      const fetchData = async () => {
        try {
          const response = await fetch(`http://191.252.212.69:3003/api/produto/${itemId}`);
  ...
  useEffect(() => {
    const fetchCategorias = async () => {
      const response = await fetch('http://191.252.212.69:3003/api/categoria');
      const data = await response.json();
      setCategorias(data);
    };
  ...
  const handleSave = async () => {
    ...
    const url = itemId ? `http://191.252.212.69:3003/api/produto/${itemId}` : 'http://191.252.212.69:3003/api/produto';
