...
const Inventory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseEstoque = await fetch('http://191.252.212.69:3003/api/estoque');
  ...
  const fetchProducts = async () => {
    try {
      const responseSaldo = await fetch(`http://191.252.212.69:3003/api/saldo/inventario?estoqueId=${estoqueId}`);
      ...
      const productRequests = dataSaldo.map(async (saldoItem) => {
        ...
        const responseProduto = await fetch(`http://191.252.212.69:3003/api/produto/${produtoId}`);
        const dataProduto = await responseProduto.json();

        const responseEstoque = await fetch(`http://191.252.212.69:3003/api/estoque/${saldoItem.estoqueId}`);
        const dataEstoque = await responseEstoque.json();
    ...
    try {
      const saveRequests = Object.entries(newSaldos).map(async ([productId, newSaldo]) => {
        ...
        if (!isNaN(newSaldo) || newSaldo === '') {
          const response = await fetch('http://191.252.212.69:3003/api/saldo', {
            method: 'POST',
            headers: {
  ...
  const generatePDF = () => {
    ...
    pdf.text('TECSAN - Estrutura para Eventos', 120, 20, { align: 'center' });
